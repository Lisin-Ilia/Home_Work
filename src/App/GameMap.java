...
public class GameMap extends JPanel {
    ...
    private final String msgHumanWin = "Human win!!!";
    private final int stateAiWin = 2;
    private final String msgAiWin = "Ai win!!!";
    private final int stateDraw = 0;
    private final String msgDraw = "Draw!!!";
    ...
    private void update(MouseEvent mouseObject) {
        ...
        mainWindow.saveLog("Human turn in [" + (cellX + 1) + ":" + (cellY + 1) + "]");
    ...
    private void showGameOverMessage(int state) {
        switch (state) {
            case stateHumanWin:
                mainWindow.saveLog(msgHumanWin);
                JOptionPane.showMessageDialog(this, msgHumanWin);
                break;
            case stateAiWin:
                mainWindow.saveLog(msgAiWin);
                JOptionPane.showMessageDialog(this, msgAiWin);
                break;
            case stateDraw:
                mainWindow.saveLog(msgDraw);
                JOptionPane.showMessageDialog(this, msgDraw);
                break;
            default:
                JOptionPane.showMessageDialog(this, "Error: Unknown state " + state);
                break;
        }
    }
    ...
    private void aiTurn() {
        ...
        map[y][x] = ai;
        mainWindow.saveLog("Ai turn in [" + (x + 1) + ":" + (y + 1) + "]");
    }
        ...
        for (int i = 0; i < mapSizeY; i++) {
            for (int j = 0; j < mapSizeX; j++) {
                if (isEmptyCell(j, i)) {
                    map[i][j] = ai;
                    if (checkWin(ai)) {
                        mainWindow.saveLog("Ai turn in [" + (j + 1) + ":" + (i + 1) + "]");
                        return true;
                    }
                    map[i][j] = emptyField;
                }
